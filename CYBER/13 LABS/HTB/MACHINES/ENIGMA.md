...
| 80/tcp   | HTTP        | nginx 1.24.0 (Ubuntu)  | Redirects to `http://enigma.htb` — requires hostname resolution via `/etc/hosts` to access.                                                                                       | The main front door — a public-facing website. When we knocked, it redirected us to a specific address, like a receptionist saying _"you need to ask for us by name."_ |
...
| 111/tcp  | RPCbind     | 2-4 (RPC #100000)      | Exposes the full RPC service map, revealing NFS (2049), `mountd`, `nlockmgr`, and `status` on dynamically assigned high ports — not independently exploitable, just NFS plumbing. | A directory board in a hallway — it doesn't do anything itself, but tells us what other rooms exist and where to find them.                                            |
